...
def bump_commit_tag(version_part, tag_message: str, config_exists=False) -> str:
    ...
    b2v_handler.run_b2v(
        version_part, target_file, tag_message=f"({{now:%d.%m.%Y}}) - {tag_message}"
    )
    after_version = file_reader.get_current_version()
    if before_version == after_version:
        exceptions.FailedVersionIncrement(
            f"Bump2Version action failed. Version is still {before_version}"
        )
    colorama_handler.print_green(
        f"bumped version: {before_version} --> {after_version}"
    )
...
def run_release_flow(version_part: str, release_message: str):
    ...
    after_version = bump_commit_tag(
        version_part, release_message, file_reader.bumpversion_config_exists()
    )
